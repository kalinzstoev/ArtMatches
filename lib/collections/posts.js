...
Posts.deny({
    update: function(userId, post, fieldNames) {
        ...
        return (_.without(fieldNames, 'description', 'title', 'category', 'text').length > 0);
    }
});
...
validateFilePost = function (post) {
    ...
    if (!post.isFileUploaded)
        errors.isFileUploaded = "Please upload at least one file";
...
Meteor.methods({
    postFileInsert: function (postAttributes) {
        ...
        if (errors.title ||  errors.category || errors.isFileUploaded)
    ...
    postTextInsert: function (postAttributes) {
        ...
        if (errors.title || errors.description || errors.type || errors.text)
            throw new Meteor.Error('invalid-post', "You must fill all required fields");
